...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public
AS $$
  SELECT bri.item_category as category, bri.item_type, SUM(bri.amount) as total_amount, COUNT(*) as item_count
  FROM public.booking_revenue_items bri
  JOIN public.bookings b ON b.id = bri.booking_id
  WHERE bri.payment_date BETWEEN p_start_date AND p_end_date AND bri.payment_date IS NOT NULL
    AND b.booking_origin = 'website' AND b.status NOT IN ('cancelled', 'declined')
  GROUP BY bri.item_category, bri.item_type ORDER BY bri.item_category, total_amount DESC;
$$;

-- =====================================================
-- 5) FIX get_revenue_by_segment
-- =====================================================
...
BEGIN
  RETURN QUERY EXECUTE format(
    'SELECT b.%I::text as segment, SUM(bri.amount) as total_revenue, COUNT(DISTINCT b.id) as booking_count,
    ...
    WHERE bri.payment_date BETWEEN $1 AND $2 AND bri.payment_date IS NOT NULL
      AND b.booking_origin = ''website'' AND b.status NOT IN (''cancelled'', ''declined'')
    GROUP BY b.%I ORDER BY total_revenue DESC',
...
-- =====================================================
-- 6) FIX get_revenue_line_items_export
-- =====================================================
CREATE OR REPLACE FUNCTION public.get_revenue_line_items_export(p_start_date date, p_end_date date)
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public
AS $$
  SELECT b.reservation_number, b.event_date, bri.payment_date, bri.payment_split, b.event_type, b.booking_type::text, b.booking_origin::text,
    b.full_name as guest_name, bri.item_category, bri.item_type, bri.amount, bri.quantity, bri.description, bri.created_at
  FROM public.booking_revenue_items bri
  JOIN public.bookings b ON b.id = bri.booking_id
  WHERE bri.payment_date BETWEEN p_start_date AND p_end_date AND bri.payment_date IS NOT NULL
    AND b.booking_origin = 'website' AND b.status NOT IN ('cancelled', 'declined')
...
-- =====================================================
-- 7) FIX get_daily_generated_revenue
-- =====================================================
CREATE OR REPLACE FUNCTION public.get_daily_generated_revenue(p_start_date date, p_end_date date)
RETURNS TABLE(generated_date date, booking_count bigint, total_generated decimal, baseline_generated decimal, cleaning_generated decimal, production_generated decimal, addon_generated decimal, tax_generated decimal, discount_generated decimal)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public
AS $$
  SELECT b.created_at::date AS generated_date, COUNT(*) AS booking_count,
