-- Fix reschedule_booking function v2
-- 1) Never change booking_type - preserve existing type from DB
-- 2) Daily: allow optional event window (start/end times) for planning purposes
-- 3) Daily event window does NOT affect blocking logic - day remains fully blocked
-- 4) Improved conflict validation

DROP FUNCTION IF EXISTS public.reschedule_booking(uuid, date, text, text, text, text);
...
BEGIN
  ...
  ELSE
    ...
    -- Check for hourly overlap using minute comparison
    ...
    WHERE event_date = p_new_date
      ...
      AND (
        ...
        < (EXTRACT(HOUR FROM v_target_end::time) * 60 + EXTRACT(MINUTE FROM v_target_end::time))
      )
      AND (
        (EXTRACT(HOUR FROM end_time::time) * 60 + EXTRACT(MINUTE FROM end_time::time))
        > (EXTRACT(HOUR FROM v_target_start::time) * 60 + EXTRACT(MINUTE FROM v_target_start::time))
      )
    LIMIT 1;
    ...
    IF FOUND THEN
      RETURN jsonb_build_object(
        ...
        'message', format('Time %s-%s overlaps with %s''s booking (%s-%s)', 
          ...
          v_conflict.start_time, v_conflict.end_time),
        'conflict', jsonb_build_object(
          'booking_id', v_conflict.id,
          'guest', v_conflict.full_name,
