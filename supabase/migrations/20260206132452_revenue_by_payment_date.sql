-- =====================================================
-- REVENUE REPORTING BY PAYMENT DATE (SPLIT DEPOSIT/BALANCE)
-- =====================================================
-- OBJECTIVE: Change revenue reporting from event_date to payment_date
-- Revenue recognition: 50% when deposit paid, 50% when balance paid
-- Internal bookings excluded from revenue tracking
-- Payroll continues using event_date (no changes to payroll functions)
-- =====================================================

-- =====================================================
-- 1) ADD PAYMENT FIELDS TO booking_revenue_items
-- =====================================================

-- Add payment_date field to track when payment was received
ALTER TABLE public.booking_revenue_items
  ADD COLUMN IF NOT EXISTS payment_date DATE;

-- Add payment_split field to track deposit vs balance split
ALTER TABLE public.booking_revenue_items
  ADD COLUMN IF NOT EXISTS payment_split TEXT 
    CHECK (payment_split IN ('deposit', 'balance', 'full'));

-- Create indexes for efficient querying
CREATE INDEX IF NOT EXISTS idx_revenue_items_payment_date 
  ON public.booking_revenue_items(payment_date);

CREATE INDEX IF NOT EXISTS idx_revenue_items_payment_split 
  ON public.booking_revenue_items(payment_split);

-- Add comments
COMMENT ON COLUMN public.booking_revenue_items.payment_date IS 
  'Date when payment was received (from deposit_paid_at or balance_paid_at)';

COMMENT ON COLUMN public.booking_revenue_items.payment_split IS 
  'Which payment this item belongs to: deposit (50%), balance (50%), or full (100% legacy)';

-- =====================================================
-- 2) REPLACE populate_booking_revenue_items WITH SPLIT LOGIC
-- =====================================================
...
BEGIN
  -- Get booking data with payment dates
  SELECT 
    *,
    deposit_paid_at::date as deposit_date,
  ...
  -- EXCLUDE internal bookings from revenue tracking
  IF v_booking.booking_origin = 'internal_admin' THEN
    RAISE NOTICE 'Skipping revenue for internal booking %', p_booking_id;
    RETURN;
  END IF;
  ...
  -- Skip if no deposit payment date available
  IF v_deposit_date IS NULL THEN
    RAISE NOTICE 'No deposit payment date for booking %, skipping revenue', p_booking_id;
    RETURN;
  END IF;
  ...
  IF v_booking.cleaning_fee > 0 THEN
    -- Determine cleaning type based on fee amount
  ...
  IF v_booking.optional_services > 0 THEN
    ...
    IF v_booking.tablecloths AND v_booking.tablecloth_quantity > 0 THEN
      ...
      END IF;
    ELSE
      -- No tablecloth breakdown, add total as misc (split 50/50)
      -- Deposit portion (50%)
      INSERT INTO public.booking_revenue_items (
        booking_id, item_category, item_type, amount,
        payment_date, payment_split, description, is_historical
...
-- =====================================================
-- 3) UPDATE get_daily_revenue TO USE payment_date
-- =====================================================
...
-- =====================================================
-- 4) UPDATE get_monthly_revenue TO USE payment_date
-- =====================================================
...
-- =====================================================
-- 5) UPDATE get_revenue_by_category TO USE payment_date
-- =====================================================
...
-- =====================================================
-- 6) UPDATE get_revenue_by_segment TO USE payment_date
-- =====================================================
...
-- =====================================================
-- 7) UPDATE get_revenue_line_items_export TO USE payment_date
-- =====================================================
...
-- =====================================================
-- 8) BACKFILL EXISTING BOOKINGS WITH NEW SPLIT LOGIC
-- =====================================================

-- Re-populate revenue items for all website bookings that have payments
DO $$
DECLARE
  v_booking_id uuid;
  v_count integer := 0;
  ...
  v_errors integer := 0;
BEGIN
  RAISE NOTICE 'Starting backfill of revenue items with payment_date split...';
  ...
  LOOP
    BEGIN
      -- Re-populate with new split logic
  ...
  RAISE NOTICE 'Successfully processed: % bookings', v_count;
  RAISE NOTICE 'Errors: %', v_errors;
  RAISE NOTICE 'Internal bookings skipped automatically by populate function';
END $$;
...
-- Check that internal bookings have no revenue items
-- SELECT COUNT(*) FROM booking_revenue_items bri
-- JOIN bookings b ON b.id = bri.booking_id
-- WHERE b.booking_origin = 'internal_admin';
-- Expected: 0

-- Check split distribution
-- SELECT payment_split, COUNT(*), SUM(amount)
-- FROM booking_revenue_items
-- GROUP BY payment_split;

-- Compare totals before and after (should be same)
-- SELECT 
--   'deposit' as split,
--   SUM(amount) as total
-- FROM booking_revenue_items 
-- WHERE payment_split = 'deposit'
-- UNION ALL
-- SELECT 
--   'balance' as split,
--   SUM(amount) as total
-- FROM booking_revenue_items 
-- WHERE payment_split = 'balance';
