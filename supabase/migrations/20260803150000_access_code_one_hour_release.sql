-- Access page redesign (Aug 2026):
-- 1. The door code is released ONE HOUR BEFORE the event start time
--    (America/New_York, DST-aware) instead of at midnight on the event day.
-- 2. While still locked, the function no longer raises
--    access_code_locked_until_event_day — it returns the booking row with
--    code/label NULL so the page can render the "not available yet" state
--    with the reservation details always visible.
-- 3. Adds start_time and access_released to the return set.
...
CREATE FUNCTION public.get_access_code_for_reservation(p_reservation_number text DEFAULT NULL::text, p_email text DEFAULT NULL::text)
 RETURNS TABLE(code text, label text, access_released boolean, booking_id uuid, reservation_number text, full_name text, email text, phone text, event_date date, start_time time without time zone, end_time time without time zone, event_type text, host_report_step text)
 LANGUAGE plpgsql
